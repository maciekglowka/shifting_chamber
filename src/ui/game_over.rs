...
use crate::actions::{Action, RestartLevelAction, RestartGameAction};
...
pub fn show_menu(
    ...
    let mut choices = vec![
        (
            "Main Menu".to_string(),
            Box::new(RestartGameAction) as Box<dyn Action>
        )
    ];
    ...
    spawn_modal(
        &mut commands,
        format!("Game Over, Score: {}", game_res.score),
        choices,
        assets.as_ref()
    );
}
...
//                 );
//             }
//             add_button(parent, assets.as_ref(), "Main Menu", CommandType::);
//         });
// }
